    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          targetProgram: data.applicability?.programs?.[0] || '',
          yearLevel: data.applicability?.yearLevels?.[0]?.toString() || '',
          semester: data.applicability?.semesters?.[0] || '',
          academicYear: data.academicYear || '',
          department: data.applicability?.departments?.[0] || '',
          effectiveDate: data.effectiveDate ? new Date(data.effectiveDate).toISOString().split('T')[0] : '',
          expiryDate: data.expiryDate ? new Date(data.expiryDate).toISOString().split('T')[0] : '',
          contactPerson: data.issuingAuthority?.signatory || '',
          contactEmail: data.issuingAuthority?.contactEmail || ''
        });
      }
    } catch (error) {
      console.error('Error fetching instruction:', error);
      setMessage('Error loading instruction data');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="main-content" style={{ maxHeight: 'calc(100vh - 64px)', overflowY: 'auto' }}>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-white">Edit Instruction</h1>
