    ...
    try {
      ...
      if (data.personalInfo && data.employmentDetails) {
        setFormData({
          ...
          employeeId: data.employmentDetails.employeeId || '',
          department: data.employmentDetails.department || '',
          position: data.employmentDetails.position || '',
          rank: data.employmentDetails.rank || '',
          status: data.employmentDetails.status || '',
          hireDate: data.employmentDetails.hireDate ? new Date(data.employmentDetails.hireDate).toISOString().split('T')[0] : '',
          degree: data.educationalBackground?.bachelors?.degree || '',
          university: data.educationalBackground?.bachelors?.university || '',
          graduationYear: data.educationalBackground?.bachelors?.year?.toString() || ''
        });
      }
    } catch (error) {
